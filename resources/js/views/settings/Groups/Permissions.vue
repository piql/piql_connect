<template>
    <div class="w-100">
        <page-heading icon="fa-user-shield" :title="$t('settings.settings.grouppermissions') + ' [ '+ group.name + ' ]'" :ingress="$t('settings.settings.grouppermdesc')" />
        <div class="card">
            <div class="card-header">
        
                <button type="button" class="btn btn-primary" @click="$bvModal.show('add-permission')">
                    <i class="fa fa-key"></i>  Add Group Permission
                </button>
                <b-modal id="add-permission" hide-footer>
                    <template v-slot:modal-title>
                   <h4> Add Group Permission </h4>
                    </template>
                    <div class="d-block">
...
<script>
    ...
    export default {
        ...
        async mounted(){
            ...
            await axios.get("/api/v1/admin/access-control/permission-groups/"+ groupId).then(res => {
        ...
        methods: {
            ...
            async addPermission(){
                if((this.permission != null) && (this.description != null)){
                    ...
                    this.response = (await axios.post('/api/v1/admin/access-control/permission-groups/'+ groupId +'/permission', {

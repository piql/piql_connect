...
class BagitUtil
{
    ...
    private $m_InputMetadataFilesSource = array();
    private $m_InputMetadataFilesDestination = array();
    ...
    public function addMetadataFile($filePathSource, $filePathDestination)
    {
        array_push($this->m_InputMetadataFilesSource, $filePathSource);
    ...
    public function createBag($outputFile)
    {
        ...
        // Add metadata
        for ($i = 0; $i < count($this->m_InputMetadataFilesSource); $i++)
        {
            $inputFileSource = $this->m_InputMetadataFilesSource[$i];
            $inputFileDestination = $this->m_InputMetadataFilesDestination[$i];
            /** let's trick the bagIt util and store the metadata content in the base dir */
            $bag->addFile($inputFileSource, 'data/metadata/' .$inputFileDestination);
            if (!file_exists($bag->getDataDirectory() . '/metadata/' . $inputFileDestination))
            {
                $this->m_ErrorMessage = 'Failed to add file to bag. Source=' . $inputFileSource . ' destination=' . $bag->getDataDirectory() . '/metadata/' . $inputFileDestination;
                return false;
            }
        }
